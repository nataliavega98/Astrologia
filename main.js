...
const checkUsername = () => {
  ...
  if (isEmpty(username)) {
    ...
    showError(
      usernameForm,
      "Tu nombre debe abarcar entre los 3 y 25 caractéres."
    );
  } else {
    valid = true;
...
const checkDate = () => {
  ...
  let signoEncontrado = signos.find((e) => {
    ...
    if (date >= new Date(fechaDeInicio) && date <= new Date(fechaDeFinal)) {
      ...
      (date >= new Date(date.getFullYear() + "-" + e.fdI1) &&
        date <= new Date(date.getFullYear() + "-" + e.fdF1)) ||
      (date >= new Date(date.getFullYear() + "-" + e.fdI2) &&
        date <= new Date(date.getFullYear() + "-" + e.fdF2))
    ) {
      valid = true;
      return e;
    } else {
...
const init = () => {
  form.addEventListener("submit", (e) => {
    ...
    checkUsername();
    if (isValidForm()) {
      return createCard(checkDate().signo);
    } else {
